...
class GenerationPipeline:
    ...
    def _hard_check_step2_structure(self, dt_data: Dict[str, Any]) -> None:
        if not self._clean_text(dt_data.get("description")):
            dt_data["description"] = f"Reduced Decision Logic Table for {dt_data.get('feature', '')}"
    ...
    def _apply_step3_packet_contract_to_final_data(
        ...
        *,
        ...
        for index, template in enumerate(templates, start=1):
            ...
            # Chỉ chức năng đăng ký mới cần tự đổi Username sang giá trị mới/duy nhất.
            # Với login, Username valid phải giữ đúng tài khoản đã tồn tại trong Step 1
            # để happy path đăng nhập thành công không bị đổi thành NewUserxxx.
            packet_feature = normalize_feature_name(self._clean_text(packet.get("feature")))
            if packet_feature == "register" and "Username" in input_fields:
                selected_field = self._clean_text(template.get("selected_invalid_field"))
                expected_text = self._clean_text(row.get("Expected"))

                is_username_fault = selected_field == "Username"
                is_username_duplicate = expected_text == "tài khoản đã tồn tại trong hệ thống"

                if not is_username_fault and not is_username_duplicate:
                    unique_username = f"NewUser{index:03d}"
                    old_username = row.get("Username")
                    row["Username"] = unique_username

                    if str(row.get("Expected")) == str(old_username):
                        row["Expected"] = unique_username
